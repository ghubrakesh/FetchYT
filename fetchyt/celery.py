...
def fetch_latest_videos():
    try:
        ...
        if response.status_code == 200:
            ...
            if video_objects:
                log.info(f"{YELLOW}bulk creating{NC}")
            ...
            log.info(f"{GREEN}fetchyt:celery :: fetch_latest_videos() celery task execution completed Successfully{NC}")
            
        else:
            log.error(f"{RED}fetchyt:celery :: fetch_latest_videos() non 200 error response : {response.json()}{NC}")

    except Exception as e:
        log.error(f"{RED}fetchyt:celery :: fetch_latest_videos() celery task execution Failed. Error: {str(e)}{NC}")
